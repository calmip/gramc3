...
class Stockage extends GramcGraf
{
        ...
        if (count($db_data) === 0) {
            ...
            foreach ($db_data as $item) {
                ...
                //$structured_data[$key][$item->getRessource()] = $item->getConso();
                    ...
                    //if( $quota1 != $quota2 )
                    //{
                    //	$this->sj->errorMessage( __METHOD__ . ':' . __LINE__ . ' incohérence dans les quotas, date = ' .  $item->getDate()->format("d F Y") . ' projet = '. $projet );
                    //}
                } else {
    ...
    */
    public function createImage($structured_data, $ressource)
    {
        // Compatibilité
        if ($ressource==null) {
            $ressource = $this->ressources_conso_group['2'];
        } // work_space
        ...
        foreach ($structured_data as $key => $item) {
            ...
            if ($structured_data[$key]['quota']>$quota_max) {
                $quota_max=$structured_data[$key]['quota'];
            }
        }
        //$this->sj->errorMessage("coucou " );
