...
class ExceptionListener
{
    ...
    public function onKernelException(GetResponseForExceptionEvent $event)
    {
        ...
        elseif ($exception instanceof AuthenticationCredentialsNotFoundException) { // problème avec REMOTE_USER
            if ($username = getenv('REMOTE_USER') ||  $server->has('REMOTE_USER') || $server->has('REDIRECT_REMOTE_USER')) { // problème avec REMOTE_USER
                ...
                $this->logger->info(
                    __METHOD__ . ":" . __LINE__ . " Unknown REMOTE_USER " .  $username . " to : " . $event->getRequest()->getRequestUri(),
                    [
                        'exception' => $exception,
        ...
        elseif ($exception instanceof HttpException) {
            ...
            $this->logger->warning(
                "Error to " .  $event->getRequest()->getRequestUri(),
                [
                    'exception' => $exception,
                    'request' => $event->getRequest()
                    ]
