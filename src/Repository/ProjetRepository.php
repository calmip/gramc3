...
class ProjetRepository extends \Doctrine\ORM\EntityRepository
{
	...
	private function countEtatPrj($etat)
	{
         return $this->getEntityManager()
         ->createQuery
        ('SELECT count(p) FROM App:Projet p WHERE ( p.etatProjet = :etat )')
    ...
    public function countEtatPrjType($etat,$type=1)
    {
         return $this->getEntityManager()
         ->createQuery
        ('SELECT count(p) FROM App:Projet p WHERE ( p.etatProjet = :etat AND p.typeProjet = :type )')
        ->setParameter('etat', Etat::getEtat($etat) )
    ...
    private function countAllPrj()
    {
         return $this->getEntityManager()
         ->createQuery
        ('SELECT count(p) FROM App:Projet p WHERE ( NOT p.etatProjet = :etat )')
    ...
    private function countAllPrjType($type)
    {
         return $this->getEntityManager()
         ->createQuery
        ('SELECT count(p) FROM App:Projet p WHERE ( NOT p.etatProjet = :etat AND p.typeProjet = :type)')
        ->setParameter('etat', Etat::getEtat('ANNULE') )
        ->setParameter(':type', $type)
        ->getSingleScalarResult();
    }
