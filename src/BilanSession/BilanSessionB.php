...
class BilanSessionB extends BilanSession
{
    ...
    protected function getLigne(Version $version, &$totaux)
    {
        ...
        if ($session_courante_A != null) {
            $version_courante_A = $this->em->getRepository(Version::class)
        ...
        $attr_heures_A             = 0;
        if ($version_courante_A != null) {
            $attr_heures_A += $version_courante_A->getAttrHeures();
        }

        $attr_heures_rallonge_A    = 0;
        if ($version_courante_A != null) {
            $attr_heures_rallonge_A += $version_courante_A->getAttrHeuresRallonge();
        }
        ...
        if ($version_courante_A != null) {
            $conso_juin = $this->sp->getConsoCalcul($projet, $date_recup->format('Y-m-d'));
            $recuperable= $this->ss->calc_recup_heures_printemps($conso_juin, $attr_heures_A);
        } else {
            $recuperable        =   0;
        }

        $stockRessource = $this->sp->getConsoRessource($projet, $ress, $full_annee_cour);
